-- ========================================================================
-- CORRECTED COMPLETE DATABASE SCHEMA FOR CAREER ADVISOR PLATFORM
-- ========================================================================
-- This is the corrected version that uses consistent naming conventions:
-- - All user foreign keys use "user_id" (not "userId")  
-- - All column names use snake_case (not camelCase)
-- - Consistent table and column naming throughout
...
-- 2. CAREER_ROADMAPS TABLE - ✅ FIXED: All columns use snake_case
CREATE TABLE IF NOT EXISTS public.career_roadmaps (
    id UUID DEFAULT uuid_generate_v4() PRIMARY KEY,
    user_id UUID REFERENCES public.profiles(id) ON DELETE CASCADE NOT NULL, -- ✅ FIXED: user_id not userId
    title TEXT NOT NULL,
    description TEXT,
    career_goal TEXT NOT NULL, -- ✅ FIXED: career_goal not careerGoal
    current_level TEXT NOT NULL CHECK (current_level IN ('beginner', 'intermediate', 'advanced')), -- ✅ FIXED: current_level not currentLevel
    duration INTEGER NOT NULL DEFAULT 12, -- months
    content JSONB NOT NULL DEFAULT '{}', -- stores AI-generated roadmap steps
    nodes JSONB NOT NULL DEFAULT '[]',
    connections JSONB NOT NULL DEFAULT '[]',
    ai_generated BOOLEAN NOT NULL DEFAULT false, -- ✅ FIXED: ai_generated not aiGenerated
    progress INTEGER NOT NULL DEFAULT 0 CHECK (progress >= 0 AND progress <= 100),
    created_at TIMESTAMPTZ DEFAULT NOW(),
    updated_at TIMESTAMPTZ DEFAULT NOW()
);

-- 3. QUIZ_RESULTS TABLE - ✅ Already uses user_id correctly
...
-- ========================================================================
-- RLS POLICIES - ✅ ALL USE user_id CONSISTENTLY
-- ========================================================================
...
-- Career roadmaps policies - ✅ FIXED: Uses user_id consistently
...
-- ========================================================================
-- PERFORMANCE INDEXES - ✅ ALL USE CORRECT COLUMN NAMES
-- ========================================================================
...
BEGIN
    RAISE NOTICE '========================================================================';
    RAISE NOTICE '✅ CORRECTED DATABASE SCHEMA CREATED SUCCESSFULLY!';
    RAISE NOTICE '========================================================================';
    RAISE NOTICE '🎯 FIXES APPLIED:';
    RAISE NOTICE '✅ career_roadmaps uses user_id (not userId)';
    RAISE NOTICE '✅ career_roadmaps uses career_goal (not careerGoal)';
    RAISE NOTICE '✅ career_roadmaps uses current_level (not currentLevel)';
    RAISE NOTICE '✅ career_roadmaps uses ai_generated (not aiGenerated)';
    RAISE NOTICE '✅ All RLS policies reference user_id consistently';
    RAISE NOTICE '✅ All foreign key constraints use user_id';
    RAISE NOTICE '✅ All indexes reference correct column names';
    RAISE NOTICE '✅ Consistent snake_case naming throughout';
    RAISE NOTICE '========================================================================';
    RAISE NOTICE '🚀 No more SQL errors - Your APIs will work seamlessly!';
    RAISE NOTICE '========================================================================';
END
$$;